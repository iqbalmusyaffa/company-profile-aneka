...
<div class="mb-6 flex items-center justify-between">
    <div>
        ...
        <h2 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Data Admin: {{ $user->name }}</h2>
    </div>
</div>
    ...
    <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="p-6 sm:p-8">
        ...
        <div class="space-y-6">
            ...
            <div class="p-4 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <p class="text-xs text-gray-500 mb-4">Kosongkan kolom ini jika Anda tidak ingin mengubah password akun admin ini.</p>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="password" class="block text-xs font-bold text-gray-700 mb-1.5">Password Baru</label>
                        <input type="password" name="password" id="password"
                               class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all @error('password') border-red-500 @enderror">
                        @error('password')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label for="password_confirmation" class="block text-xs font-bold text-gray-700 mb-1.5">Konfirmasi Password Baru</label>
                        <input type="password" name="password_confirmation" id="password_confirmation"
                               class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all">
                    </div>
                </div>
            </div>
        </div>
