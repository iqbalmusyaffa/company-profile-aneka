...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-gray-900">{{ isset($category) ? 'Edit Kategori' : 'Tambah Kategori' }}</h2>
    </div>
    <a href="{{ route('admin.categories.index') }}" class="text-gray-500 hover:text-gray-700 font-medium transition-colors flex items-center">
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden max-w-3xl">
    <form action="{{ isset($category) ? route('admin.categories.update', $category->id) : route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
        @csrf
        @if(isset($category))
            @method('PUT')
        @endif
        ...
        <!-- Nama Kategori -->
        <div>
            <label for="name" class="block text-sm font-semibold text-gray-700 mb-1">Nama Kategori <span class="text-red-500">*</span></label>
        ...
        <!-- Gambar Kategori -->
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-3">Gambar Kategori</label>
            @if(isset($category) && $category->hasMedia('categories'))
                <div class="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-2xl flex justify-center items-center relative group overflow-hidden">
                    ...
                    <img src="{{ $category->getFirstMediaUrl('categories') }}" alt="Current Image" class="h-24 object-contain relative z-0">
                </div>
            @endif
