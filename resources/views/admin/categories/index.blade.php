...
<div class="mb-8 flex flex-col md:flex-row justify-between md:items-center gap-4">
    <div>
        <h2 class="text-3xl font-extrabold text-gray-900 tracking-tight">Kategori Produk</h2>
        <p class="text-gray-500 text-sm mt-1">Kelola dan kelompokkan material bangunan Anda dengan mudah.</p>
    </div>
    <div class="flex flex-wrap gap-3">
        <a href="{{ route('admin.categories.create') }}" class="bg-gradient-to-r from-primary-600 to-primary-500 hover:from-primary-700 hover:to-primary-600 text-white font-semibold py-2.5 px-5 rounded-xl shadow-lg shadow-primary-500/30 transition-all flex items-center">
            ...
            <tbody class="bg-white divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/80 transition-colors group">
                    ...
                    <td class="px-8 py-5 whitespace-nowrap">
                        <div class="flex items-center">
                            @if($category->hasMedia('categories'))
                                <img class="h-12 w-12 rounded-xl object-cover border border-gray-200 shadow-sm" src="{{ $category->getFirstMediaUrl('categories') }}" alt="">
                            @else
                            ...
                            @endif
                            <div class="ml-4">
                                <div class="text-sm font-bold text-gray-900 group-hover:text-primary-600 transition-colors">{{ $category->name }}</div>
                                <div class="text-xs text-gray-500 mt-1 font-mono bg-gray-100 px-2 py-0.5 rounded inline-block">Slug: {{ $category->slug }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-8 py-5">
                    ...
                    <td class="px-8 py-5 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('admin.categories.edit', $category->id) }}" class="text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 p-2 rounded-xl transition-colors" title="Edit">
                ...
                <tr>
                    <td colspan="5" class="px-8 py-12 text-center">
                        ...
                        <p class="text-gray-500 text-sm">Tambahkan kategori pertama Anda.</p>
                        <a href="{{ route('admin.categories.create') }}" class="mt-4 inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-xl text-white bg-primary-600 hover:bg-primary-700 transition-colors">
                            Tambah Kategori
                        </a>
                    </td>
                </tr>
