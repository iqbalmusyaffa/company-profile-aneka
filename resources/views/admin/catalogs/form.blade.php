...
<div class="mb-6 flex items-center justify-between">
    ...
    <a href="{{ route('admin.catalogs.index') }}" class="btn-secondary">
...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 sm:p-8">
    <form action="{{ isset($catalog) ? route('admin.catalogs.update', $catalog->id) : route('admin.catalogs.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <!-- Nama Katalog -->
            <div>
                <label for="title" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">Nama Katalog <span class="text-red-500">*</span></label>
                <input type="text" name="title" id="title" value="{{ old('title', $catalog->title ?? '') }}" required
                    class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:bg-white dark:focus:bg-gray-800 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all text-gray-900 dark:text-white"
            ...
            <!-- Kategori -->
            <div>
                <label for="category" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">Kategori File</label>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Ketik baru atau pilih dari daftar.</p>
                @error('category') <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span> @enderror
            </div>

            <!-- Tanggal Berlaku -->
            <div>
                <label for="publish_date" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">Tanggal Berlaku</label>
                <input type="date" name="publish_date" id="publish_date" value="{{ old('publish_date', isset($catalog) && $catalog->publish_date ? $catalog->publish_date->format('Y-m-d') : '') }}"
                    class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:bg-white dark:focus:bg-gray-800 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all text-gray-900 dark:text-white [&::-webkit-calendar-picker-indicator]:dark:invert">
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Kosongkan jika tidak ada tanggal.</p>
        ...
        <!-- File Upload PDF -->
        <div class="mb-6">
            <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">File PDF Katalog <span class="text-red-500">*</span></label>
            
            <div class="flex items-center justify-center w-full">
                <label for="pdf_file" class="flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900 hover:bg-gray-100 dark:border-gray-700 dark:hover:border-gray-600 transition-all group relative overflow-hidden">
                    <div class="flex flex-col items-center justify-center pt-5 pb-6 text-center px-4 z-10">
                        ...
                        <p class="mb-1 text-sm text-gray-500 dark:text-gray-400"><span class="font-bold text-primary-600 dark:text-primary-400">Klik untuk unggah PDF</span> atau drag and drop</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">PDF (Maks. 10MB)</p>
                        
                        <div id="file-name-display" class="mt-3 hidden">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-primary-100 text-primary-800 dark:bg-primary-900/30 dark:text-primary-400">
            ...
            @if(isset($catalog) && $catalog->hasMedia('catalogs'))
                <div class="mt-3 flex items-center gap-2 text-sm text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 p-3 rounded-lg border border-green-100 dark:border-green-800/30">
        ...
        <div class="mb-8">
            <label class="relative inline-flex items-center cursor-pointer">
                ...
                <span class="ml-3 text-sm font-bold text-gray-700 dark:text-gray-300">Tampilkan di Halaman Utama (Aktif)</span>
            </label>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-17">Hanya katalog dengan status Aktif yang akan terlihat oleh pengunjung.</p>
        </div>

        <div class="flex justify-end gap-3 pt-6 border-t border-gray-100 dark:border-gray-700">
            <a href="{{ route('admin.catalogs.index') }}" class="px-6 py-3 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 font-bold rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Batal</a>
